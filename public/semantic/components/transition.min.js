            ...
            f = {
              initialize: function () {
                ;(d = f.get.settings.apply(l, b)),
                  ...
                  (s = m.data(n) || f),
                  (a = f.get.animationEndEvent()),
                  !1 === (w = w && f.invoke(y)) &&
                    (f.verbose('Converted arguments into settings object', d),
                    d.interval ? f.delay(d.animate) : f.animate(),
                    f.instantiate())
              },
              instantiate: function () {
                f.verbose('Storing instance of module', f), (s = f), m.data(n, s)
              },
              destroy: function () {
                f.verbose('Destroying previous module for', l), m.removeData(n)
              },
              refresh: function () {
              ...
              forceRepaint: function () {
                ...
                var n = m.parent(),
                  e = m.next()
                0 === e.length ? m.detach().appendTo(n) : m.detach().insertBefore(e)
              },
              repaint: function () {
              ...
              delay: function (n) {
                var e,
                  i =
                    (i = f.get.animationDirection()) ||
                    (f.can.transition() ? f.get.direction() : 'static')
                ;(n = n !== S ? n : d.interval),
                  (e =
                    ('auto' == d.reverse && i == c.outward) || 1 == d.reverse
                      ? (r.length - t) * d.interval
                      : t * d.interval),
                  f.debug('Delaying animation by', e),
                  setTimeout(f.animate, e)
              },
              animate: function (n) {
                if (((d = n || d), !f.is.supported())) return f.error(e.support), !1
                if ((f.debug('Preparing animation', d.animation), f.is.animating())) {
                  if (d.queue)
                    return (
                      !d.allowRepeats && f.has.direction() && f.is.occurring() && !0 !== f.queuing
                        ? f.debug(
                  ...
                  if (!d.allowRepeats && f.is.occurring())
                    return (
                      f.debug(
                  ...
                  f.debug('New animation started, completing previous early', d.animation),
                    s.complete()
                }
                f.can.animate()
                  ? f.set.animating(d.animation)
                  : f.error(e.noAnimation, d.animation, l)
              },
              reset: function () {
              ...
              queue: function (n) {
                f.debug('Queueing animation of', n),
                  (f.queuing = !0),
                  m.one(a + '.queue' + o, function () {
                    ;(f.queuing = !1), f.repaint(), f.animate.apply(this, d)
                  })
              },
              complete: function (n) {
                f.debug('Animation complete', d.animation),
              ...
              force: {
                visible: function () {
                  var n = m.attr('style'),
                    ...
                    o = n === S || '' === n
                  a !== i
                    ? (f.verbose('Overriding default display to show element', i),
                      m.attr('style', t))
                    : o && m.removeAttr('style')
                },
                hidden: function () {
                  var n = m.attr('style'),
                    e = m.css('display'),
              ...
              set: {
                ...
                duration: function (n, e) {
                  ;(!(e = 'number' == typeof (e = e || d.duration) ? e + 'ms' : e) && 0 !== e) ||
                    (f.verbose('Setting animation duration', e), m.css({ 'animation-duration': e }))
                },
                direction: function (n) {
                  ;(n = n || f.get.direction()) == c.inward ? f.set.inward() : f.set.outward()
                },
                looping: function () {
                  f.debug('Transition set to loop'), m.addClass(c.looping)
                },
                hidden: function () {
                  m.addClass(c.transition).addClass(c.hidden)
                },
                inward: function () {
                  f.debug('Setting direction to inward'),
                    m.removeClass(c.outward).addClass(c.inward)
                },
                outward: function () {
                  f.debug('Setting direction to outward'),
                    m.removeClass(c.inward).addClass(c.outward)
                },
                visible: function () {
                  m.addClass(c.transition).addClass(c.visible)
                },
              },
              start: {
                animation: function (n) {
                  ;(n = n || f.get.animationClass()),
                    f.debug('Starting tween', n),
                    m.addClass(n).one(a + '.complete' + o, f.complete),
                    d.useFailSafe && f.add.failSafe(),
                    f.set.duration(d.duration),
                    d.onStart.call(l)
                },
              },
              save: {
                animation: function (n) {
                  f.cache || (f.cache = {}), (f.cache.animation = n)
                },
                displayType: function (n) {
                  'none' !== n && m.data(i.displayType, n)
                },
                transitionExists: function (n, e) {
                  ;(C.fn.transition.exists[n] = e),
                    f.verbose('Saving existence of transition', n, e)
                },
              },
              restore: {
                conditions: function () {
                  var n = f.get.currentAnimation()
                  n && (m.removeClass(n), f.verbose('Removing animation class', f.cache)),
                    f.remove.duration()
                },
              },
              add: {
                failSafe: function () {
                  var n = f.get.duration()
              ...
              remove: {
                ...
                direction: function () {
                  m.removeClass(c.inward).removeClass(c.outward)
                },
                duration: function () {
                ...
                visible: function () {
                  m.removeClass(c.visible)
                },
                looping: function () {
                  f.debug('Transitions are no longer looping'),
                    f.is.looping() && (f.reset(), m.removeClass(c.looping))
                },
                transition: function () {
                  m.removeClass(c.visible).removeClass(c.hidden)
                },
              },
              get: {
                ...
                animationClass: function (n) {
                  var e = n || d.animation,
                    i = f.can.transition() && !f.has.direction() ? f.get.direction() + ' ' : ''
                ...
                duration: function (n) {
                  return (
                    !1 === (n = n || d.duration) && (n = m.css('animation-duration') || 0),
                    'string' == typeof n
                      ? -1 < n.indexOf('ms')
                        ? parseFloat(n)
                        : 1e3 * parseFloat(n)
                ...
                displayType: function (n) {
                  return (
                    ...
                    d.displayType
                      ? d.displayType
                      : (n && m.data(i.displayType) === S && f.can.transition(!0),
                        m.data(i.displayType))
                  )
                },
                userStyle: function (n) {
                  return (n = n || m.attr('style') || '').replace(/display.*?;/, '')
                },
                    ...
                    if (
                      ...
                      u ||
                        ((u = t
                          ...
                          .show()
                          .css('display')),
                        f.verbose('Determining final display state', u),
                        f.save.displayType(u)),
                      t.remove(),
                      a != o)
                    )
                      f.debug('Direction exists for animation', s), (r = !0)
                    else {
                      if ('none' == a || !a) return void f.debug('No animation defined in css', s)
                      f.debug('Static animation found', s, u), (r = !1)
                    }
              ...
              is: {
                animating: function () {
                  return m.hasClass(c.animating)
                },
                inward: function () {
                  return m.hasClass(c.inward)
                },
                outward: function () {
                  return m.hasClass(c.outward)
                },
                looping: function () {
                  return m.hasClass(c.looping)
                },
                occurring: function (n) {
                  return (
                    (n = '.' + (n = n || d.animation).replace(' ', '.')), 0 < m.filter(n).length
                  )
                },
                visible: function () {
                  return m.is(':visible')
              ...
              stop: function () {
                f.debug('Stopping current animation'), m.triggerHandler(a)
              },
              stopAll: function () {
                f.debug('Stopping all animation'), f.remove.queueCallback(), m.triggerHandler(a)
              },
              clear: {
                queue: function () {
              ...
              enable: function () {
                f.verbose('Starting animation'), m.removeClass(c.disabled)
              },
              disable: function () {
                f.debug('Stopping animation'), m.addClass(c.disabled)
              },
              setting: function (n, e) {
                if ((f.debug('Changing setting', n, e), C.isPlainObject(n))) C.extend(!0, d, n)
                else {
                  if (e === S) return d[n]
              ...
              internal: function (n, e) {
                if (C.isPlainObject(n)) C.extend(!0, f, n)
                else {
                  if (e === S) return f[n]
                  f[n] = e
                }
              ...
              error: function () {
                d.silent ||
                  ((f.error = Function.prototype.bind.call(console.error, console, d.name + ':')),
